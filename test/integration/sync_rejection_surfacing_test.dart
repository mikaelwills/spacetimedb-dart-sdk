...
void main() {
  ...
  group('Sync rejection surfacing', () {
    ...
    Future<void> waitForSyncState(bool Function(SyncState) predicate) async {
      ...
      while (!predicate(env.subManager.syncState)) {
        if (DateTime.now().isAfter(deadline)) {
          fail(
            'syncState never satisfied predicate: '
    ...
    test(
      'real server rejection is rolled back and surfaced',
      () async {
        ...
        expect(
          state.recentFailures.single.reducerName,
    ...
    test(
      'mixed batch: rejection does not poison the queue and reports partially',
      () async {
    ...
    test(
      'success and rejection flushes are distinguishable via SyncState',
      () async {
        final ts = DateTime.now().microsecondsSinceEpoch;
