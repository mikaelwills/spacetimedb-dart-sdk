...
This release is one campaign: making the client cache a trustworthy local replica. A consumer app should be able to paint from cache instantly — across reconnects, across unsubscribes, across restarts — and let the server reconcile it afterwards, never the other way round. The reconnect work (ownership generations), the unsubscribe work (opt-in row retention), and the persistence work (query tags on disk) below are the three legs of that same goal.

### Added

- **Opt-in row retention across unsubscribe: `retainRowsOnUnsubscribe`.** By default, ending a subscription evicts the rows it delivered — matching the upstream Rust/TS SDKs, and the right call for zone-shaped consumers where unsubscribe means "free that memory". Offline-first apps that subscribe per screen want the opposite: leaving a screen should not discard what the user was just looking at. With `retainRowsOnUnsubscribe: true` on the client, rows whose last owning query set departs are kept and tagged with a stable hash of that set's queries. The next subscribe of the same queries paints from those rows immediately and then reconciles against the arriving snapshot: rows the server re-delivers become owned again, rows it no longer returns are evicted (so state deleted server-side while the app was away — TTL sweeps, remote cleanups — converges instead of lingering). Tags are persisted alongside the offline snapshot (a `__tags__<table>` sidecar via the existing storage API; legacy snapshots load unchanged) so a cold start paints offline-first too. A snapshot for one query set never evicts rows retained under a different set's tag — scoped subscriptions stay isolated. With the flag off nothing changes: no tags are ever written, loaded, or consulted, and unsubscribe evicts exactly as before.

- **Per-table eviction counters by cause.** `TableCache` exposes `unsubscribeEvictionCount`, `reconcileEvictionCount`, and `sweepEvictionCount`, so a consumer watching `rows` can distinguish an explained shrink (rows removed by its own unsubscribe, a reconcile, or the stale-data sweep) from genuinely anomalous row loss instead of alarming on every drop.
...
- **The replication engine of `TableCache` is now `@internal`.** `spacetimedb_sdk.dart` exports `table_cache.dart` wholesale, so every member of the cache's ownership and replication machinery was public API — including the paths that write rows without establishing ownership. 28 engine members are now annotated `@internal`: the subscription-lifecycle and delta-application paths (`markSubscribed`, `markSubscribeFailed`, `applyTransactionUpdate`, `applyTransactionUpdateAndCollectKeys`, `applyServerDelta`, `applySnapshot`, `applyInitialData`, `applyDeletes`, `dropQuerySet`), the ownership map (`ownedKeys`, `clearOwners`, `ownerEntryCount`, `ownershipImbalanceCount`, `primaryKeys`), the direct mutators (`insertRow`, `updateRow`, `deleteRow`, `insertServerOwnedRow`, `removeRowsWhere`, `removeNoPkRow`, `clearNoPkRequestTag`, `clearUntaggedNoPkRows`), the persistence pair (`toSerializable`, `loadFromSerializable`), and `decoder`, `getRow`, `emitBatch`, `dispose`.

  **Minor bump: nothing is removed, renamed, or re-signatured, and there is no runtime behaviour change of any kind** — `@internal` is analyzer metadata. The read surface consumers actually use is untouched and remains public: `tableName`, `hasPrimaryKey`, `isEvent`, `rows`, `lastBatch`, `onInsert`, `onUpdate`, `onDelete`, `subscribed`, `isSubscribed`, `count`, `find`, `iter`, `rowNotifier`, `clear`, and the constructor. Generated code is unaffected — it names `TableCache<T>` in return types and calls none of the internalized members, and the class itself is deliberately not annotated.

  `ClientCache.serializeAllTables()` and `ClientCache.loadSerializedTables()` are `@internal` for the same reason: they are thin wrappers that call `toSerializable`/`loadFromSerializable` on every registered table, so leaving them public would have re-exposed the same owner-less write path the annotations above close. `ClientCache`'s registry and lookup surface is unchanged and remains public: `registerDecoder`, `getTableByTypedName`, `getTableByName`, `allTables`, `hasBuilder`, `isEventTable`, `registeredTableNames`, `activatedTableNames`, `tableCount`, and `clearAll`.

  The only consumer-visible effect is an `invalid_use_of_internal_member` **warning** (not an error — such code still compiles) for code outside the package that calls the replication engine directly. The common case is a test that seeds cache state via `insertRow`; the supported paths are the `OptimisticChange.*` factories for mutation and `OfflineStorage` for persistence. A consumer whose CI escalates warnings to errors can unblock with a one-line analyzer override: under `analyzer:` → `errors:`, set `invalid_use_of_internal_member: ignore`.

### Fixed

- **Reconnecting no longer destroys row ownership, so cached rows the server never deleted are no longer evicted.** Data loss. `_onReconnected` used to call `clearOwners()` on every table before resubscribing, which collapsed two opposite situations onto one representation: a disk-loaded row that was never subscribed (must be kept) and a previously-owned row awaiting re-claim (may be stale) both reported zero owners. For the whole resubscribe window every ownership guard in the SDK was inert and the eviction decision was a guess. Ownership is now moved aside into a pending generation instead of being destroyed — `ownedKeys` answers from the previous generation until a query set re-claims the row, so guards stay live throughout and the two states are distinguishable by construction.

  Three consequences, each covered by a test that fails on the old code:
  - A connection bounce during the resubscribe window no longer empties the cache. A `Disconnected` force-completes every pending subscribe waiter, and the waiter carried no applied/aborted outcome, so a force-completed waiter was indistinguishable from an applied one; the post-resubscribe gate additionally read the *current* connection state, already reconnected after the bounce. Eviction therefore ran against ownership that nothing had rebuilt. Eviction now requires each query set's `SubscribeApplied` to have genuinely applied, tracked per reconnect.
  - Rows belonging to a query set dropped by `forgetQuerySet` are no longer evicted on the next reconnect. Their set is never re-subscribed, so nothing can re-claim them; a leftover is now only treated as a server-side delete when some *live* query set could have re-claimed it and did not.
  - Reconnect reconciliation no longer scans the rows held. The candidate build read every primary key of every table and the eviction ran a full key-space scan; both are gone. Opening and closing a generation over a 500,000-row disk-loaded cache is now measured at 3µs and 10µs respectively, against 44ms for the equivalent phase before. Work is now proportional to the rows the server re-reported plus the rows actually evicted.

  `SubscribeApplied`'s snapshot persist also now passes an `onlyTables` filter, matching the seven other persist sites; it was the only unfiltered one, so any table wrongly emptied in memory could be serialized over a good on-disk snapshot.

- **A committed row is no longer deleted when an offline-queued mutation is replayed after the row already arrived.** Data loss, present since 2.2.0. When a queued mutation is retried and the server has already applied it, the reducer is a no-op and the resulting `Committed()` carries no rows for that primary key. The optimistic-insert cleanup in `SubscriptionManager` read that as a mispredicted insert and deleted the row — including when the real row had already been delivered by `SubscribeApplied`, so a message that reached the server vanished from the local cache. The cleanup now skips any row a live query set owns; a genuinely mispredicted insert is unowned and is still removed. At-least-once retry semantics are unchanged.
