...
- **`ConnectionId` and `Uuid` codegen support.** Schemas using either type now generate; previously the whole schema failed with `Unknown algebraic type shape: [U128]`. Both are 16-byte types usable as a primary key: `ConnectionId` renders as hex, `Uuid` as RFC 4122.
...
- **`forgetQuerySet`.** Drops a subscription without evicting its rows.

- **Per-table eviction counters by cause** on `TableCache`: `unsubscribeEvictionCount`, `reconcileEvictionCount`, `sweepEvictionCount`.

- **`SpacetimeDbConnection.clearToken()`.** Drops the connection's in-memory auth token so a rejected token can be recovered in place: clear, reconnect, get a fresh identity. Previously `updateToken` could only replace a token, so a `401` required rebuilding the client.

- **`SpacetimeDbConnection.setKeepAliveWorkInFlight(bool)`.** Tells the app-level keep-alive monitor a long server-side operation is in flight, so a silent inbound stream is not read as a dead socket. No-op unless `appLevelKeepAlive` is enabled.

### Changed

- **The replication engine of `TableCache` is now `@internal`** — 28 members covering the subscription lifecycle, delta application, the ownership map, the direct mutators, and the persistence pair. `ClientCache.serializeAllTables()` and `loadSerializedTables()` likewise. `@internal` is analyzer metadata only: nothing is removed, renamed, or re-signatured, and there is no runtime behaviour change. The read surface consumers use stays public, and generated code is unaffected. Calling the engine from outside the package now warns `invalid_use_of_internal_member`; the supported paths are the `OptimisticChange.*` factories and `OfflineStorage`.

### Fixed

- **Reconnecting no longer destroys row ownership, so cached rows the server never deleted are no longer evicted.** Data loss. `_onReconnected` called `clearOwners()` before resubscribing, making a never-subscribed disk-loaded row indistinguishable from a previously-owned row awaiting re-claim. Ownership now moves into a pending generation instead, so guards stay live through the resubscribe window. Also: a connection bounce mid-resubscribe no longer empties the cache, `forgetQuerySet` rows survive the next reconnect, and reconciliation no longer scans the full key space (3µs/10µs to open and close a generation over 500,000 rows, against 44ms before).

- **A committed row is no longer deleted when an offline-queued mutation is replayed after the row already arrived.** Data loss, present since 2.2.0. A retried mutation the server had already applied returned `Committed()` with no rows for that key, which the optimistic-insert cleanup read as a mispredicted insert. The cleanup now skips rows a live query set owns; genuinely mispredicted inserts are unowned and still removed.

- **The keep-alive monitor no longer declares a live connection dead during a long subscription assemble.** A snapshot arrives as one inbound frame, so there is no traffic to re-arm the idle timer while the server assembles it. The in-flight signal is now asserted while any query set awaits `SubscribeApplied`. Detection is deferred, not disabled: after `KeepAliveMonitor.maxDeferredPings` (3) consecutive unanswered pings the connection is declared dead regardless.
